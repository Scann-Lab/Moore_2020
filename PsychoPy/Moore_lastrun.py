...
expName = 'Moore'  # from the Builder filename that created this script
...
filename = _thisDir + os.sep + u'data/%s_%s_%s_%s' % (expInfo['participant'], expInfo['run'], expInfo['group'], expInfo['version'])

# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='C:\\Users\\Public\\Moore_fMRI\\PsychoPy\\Moore_lastrun.py',
...
# update component parameters for each repeat
tic = time()
win.callOnFlip(thisExp.addData, "main_task_start", tic)
thisExp.addData('real_timestamp_trial_start', datetime.datetime.today().strftime('%Y-%m-%d-%H:%M:%S.%f'))
# keep track of which components have finished
...
for thisComponent in Learning_3Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
toc = time()
win.callOnFlip(thisExp.addData, "main_task_end", toc)
win.callOnFlip(thisExp.addData, "main_task_duration", (toc - tic))
thisExp.addData('real_timestamp_trial_end', datetime.datetime.today().strftime('%Y-%m-%d-%H:%M:%S.%f'))
