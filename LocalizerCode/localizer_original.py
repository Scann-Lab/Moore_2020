...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='G:\\My Drive\\SCANN Lab\\All_Shared\\Measure_Collection\\Computer_Tasks\\LocalizerCode\\localizer.py',
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if scanner_t.status == STARTED and not waitOnFlip:
            ...
            if len(theseKeys):
                ...
                if "escape" == theseKeys:
                    endExpNow = True
                scanner_t.keys = theseKeys.name  # just the last key pressed
                scanner_t.rt = theseKeys.rt
                # a response ends the routine
                continueRoutine = False
        ...
        if len(theseKeys):
            ...
            trials.addData['real_time_stamp'] = now
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if repeat_response.status == STARTED and not waitOnFlip:
            theseKeys = repeat_response.getKeys(keyList=['b', 'r', 'g', 'y'], waitRelease=False)
            if len(theseKeys):
                theseKeys = theseKeys[0]  # at least one key was pressed
                ...
                if "escape" == theseKeys:
                    endExpNow = True
                repeat_response.keys = theseKeys.name  # just the last key pressed
                repeat_response.rt = theseKeys.rt
